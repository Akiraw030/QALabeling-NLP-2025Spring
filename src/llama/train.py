...
# --- Configuration ---
class CFG:
    model_name = 'meta-llama/Llama-3.2-1B'  # Llama 3.2 1B for efficient finetuning
    pooling_strategy = 'arch1_6groups' 
    trust_remote_code = False  # Llama doesn't require trust_remote_code
...
def train_fn(train_loader, model, optimizer, scheduler, epoch, scaler):
    ...
    # Use bfloat16 for mixed precision (no GradScaler needed)
...
def valid_fn(valid_loader, model):
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            # Convert float16 to float32 for sigmoid and numpy conversion
            y_preds = y_preds.float().sigmoid().cpu().numpy()
...
if __name__ == '__main__':
    ...
    tokenizer = AutoTokenizer.from_pretrained(CFG.model_name, padding_side='right', trust_remote_code=CFG.trust_remote_code)
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        # If backbone is frozen, only optimize trainable parameters (heads).
        trainable_params = [p for p in model.parameters() if p.requires_grad]
        optimizer = AdamW(
            [{'params': trainable_params, 'lr': CFG.head_lr}],
        ...
        for epoch in range(CFG.epochs):
            avg_loss = train_fn(train_loader, model, optimizer, scheduler, epoch, None)
            ...
            if score > best_score:
                ...
                if CFG.use_kfold:
                    save_name = f"llama_fold{fold}_best.pth"
                else:
                    save_name = f"llama_single_run_best.pth"
